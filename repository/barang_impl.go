...
func (repository *BarangRepoImpl) Save(ctx context.Context, tx *sql.Tx, barang domain.Barang, idUser int) (domain.Barang, error) {
	...
	if barang.ProfitProd < 0 {
		return barang, fmt.Errorf("harga profit tidak boleh dibawah harga jual")
	}
	return barang, nil
}

func (repository *BarangRepoImpl) Update(ctx context.Context, tx *sql.Tx, barang domain.Barang, idUser int) (domain.Barang, error) {
...
func (repository *BarangRepoImpl) FindByNameRegister(ctx context.Context, tx *sql.Tx, kodeBarang string, barangName string, idUser int) (domain.Barang, error) {
	...
	if rows.Next() {
		rows.Scan(&barang.Id, &barang.IdUser, &barang.NameProd, &barang.HargaProd, &barang.Keterangan, &barang.Stok, &barang.KodeBarang, &barang.JualProd, &barang.ProfitProd)
		if kodeBarang == barang.KodeBarang {
			return barang, fmt.Errorf("%s %s Sudah Digunakan, Mohon Untuk Cek Di Inventory", kodes, kodeBarang)
		}
...
func (repository *BarangRepoImpl) FindByNameUpdate(ctx context.Context, tx *sql.Tx, kodeBarang string, barangName string, idUser int) (domain.Barang, error) {
	SQL := "select id, iduser, nameprod, HargaProd, keterangan, stok, kodebarang, jualprod, profitprod from barang where nameprod = $1 and iduser = $3 or kodebarang = $2 and iduser = $3"
	rows, err := tx.QueryContext(ctx, SQL, barangName, kodeBarang, idUser)
	helper.PanicError(err)
	namas := "Nama Barang"
